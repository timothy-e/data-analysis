from read_data import SpotifyReader, Fitbit
...
if __name__ == "__main__":
    ...
    read_weights = Fitbit.WeightReader.get_data()
    weight_times = [weight.timestamp for weight in read_weights]
    weights = [weight.weight for weight in read_weights]

    weight_smoothing = 5
    smoothed_weights = smooth(weights, weight_smoothing)
    ...
    sleep_data = Fitbit.SleepScoreReader.get_data()
    sleep_timestamps = [score.timestamp for score in sleep_data]
    sleep_scores = [score.overall_score for score in sleep_data]
    sleep_heart_rates = [score.resting_heart_rate for score in sleep_data]
    sleep_restlessnesses = [score.restlessness for score in sleep_data]
    ...
    for i in range(len(axs)):
        twin_ax = axs[i].twinx()
        if (i == 0):
            twin_ax.plot([datetime.fromtimestamp(x) for x in smooth(weight_times, 2)], smooth(weights, 2), 'b', linewidth=2, label='total weight')
        if (i == 1):
